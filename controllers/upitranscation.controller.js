import UpiTransaction from "../models/UpiTransaction.model.js";
import generateChecksum from "../utils/cheskum.js";

export const initiateUPIInstant = async (req, res) => {
  try {
    const {
    ...
    if (!merchantId || !merchantKey) {
      return res.status(400).json({ code: "1", msg: "Missing merchant headers", data: { error: "Missing headers" } });
    }

    if (!merchantTransactionId || !amount || !name || !mobile || !channel || !checksum) {
      return res.status(400).json({ code: "1", msg: "Missing required fields", data: { error: "Missing body parameters" } });
    }

    if (merchantTransactionId.length > 18) {
      return res.status(400).json({ code: "1", msg: "Transaction ID too long", data: { error: "Transaction ID exceeds 18 characters" } });
    }
    ...
    if (checksum !== expectedChecksum) {
      return res.status(401).json({ code: "1", msg: "Invalid checksum", data: { error: "Checksum validation failed" } });
    }
    ...
    return res.status(500).json({ code: "1", msg: "Server error", data: { error: error.message } });
  }
};

export const checkTxnStatus = async (req, res) => {
  try {
    ...
    if (!merchantId || !merchantKey) {
      return res.status(400).json({ code: "1", msg: "Missing headers", data: { error: "Missing X-MERCHANT-ID or X-MERCHANT-KEY" } });
    }

    if (!merchantTransactionId) {
      return res.status(400).json({ code: "1", msg: "Missing body", data: { error: "merchantTransactionId is required" } });
    }

    const txn = await UpiTransaction.findOne({ merchantTransactionId });

    if (!txn) {
      return res.status(404).json({ code: "1", msg: "Not Found", data: { error: "Transaction not found" } });
    }
...
export default {initiateUPIInstant,checkTxnStatus};